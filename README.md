...
見本と描画の中心を合わせてから、輪郭、線の傾き、大きさ、縦横比を評価します。「輪郭線と影」では、完成見本とは別に形状だけ・投影影だけの評価用画像を生成し、同じ中心合わせを影にも適用します。これにより、立体に対する影の方向・位置・広がりを独立した項目として評価します。描いた位置そのものではなく、形・大きさ・相対的な位置関係を振り返るための補助指標です。
...
最新版のChromium系ブラウザを自動テストの対象としています。マウス操作はE2Eテストで確認し、タッチ・液晶ペン入力は実機での手動確認対象としています。Firefox、Safari、古いブラウザの動作は現時点では保証していません。
