...
ランダムな見本生成を直接`Math.random()`へ依存させず、シード付き乱数を利用します。これにより、不具合の再現、固定条件でのテスト、お気に入りからの同一立体の再練習が可能になります。

### ブラウザ内で処理を完結する

画像、描画データ、設定、お気に入りは外部サーバーへ送信せず、端末内で描画・評価・保存します。氏名や連絡先などの個人情報の入力を必要としない構成です。サイトの利用状況を把握するアクセス解析だけは例外とし、Google Analyticsへアクセス情報を送信します。
...
練習中の出題、試行結果、検証メッセージ、開始・終了操作は`usePracticeSession`が管理します。セッションの状態遷移は`practiceSessionReducer`、時間の計測は`usePracticeTimer`へ分けています。練習開始時には、正規化済み設定、実際に使用したシード、生成バージョン、生成済み出題を`PracticeSessionSnapshot`として確定します。フックの返り値は`current`、`timer`、`results`、`validation`、`actions`へ分け、呼び出し側で状態と操作の境界を把握しやすくしています。
...
結果画面でだけ使う選択中の結果、比較表示モード、重ね合わせの不透明度、画像保存処理は`ResultsScreen`が管理します。全結果の保存画像は1結果を1行で表示し、ヘッダーへ合計・平均描画時間、総合点、各評価項目の平均を出力します。`app/page.tsx`には、複数画面や複数機能の連携に必要な状態だけを残しています。
...
練習開始時の条件と出題列をまとめた再現用データです。

- スナップショット保存形式のバージョン
- 実際に使用したシード
- 出題生成ロジックのバージョン
- 正規化済みの`Settings`
- 生成済みの`ShapePrompt`一覧

同じシードと設定から出題を再生成できるだけでなく、生成ロジックを将来変更した場合も、保存済みの出題値から当時の見本を再現できる構成にしています。現在はセッション中のメモリに保持し、練習履歴としては永続化していません。

### `Favorite`

お気に入りは`prompt`として生成済みの`ShapePrompt`を持ちます。立体の種類、比率、カメラ角度、物体の回転、光源を保存し、時間、描画モード、ペン、見本表示などの練習設定は保持しません。

お気に入り画面では「立体名・同じ種類内の連番・光源方向」を表示名とし、練習対象をチェックボックスで複数選択できます。開始すると、選択した`ShapePrompt`を変更せずに一覧順で出題し、現在の練習設定を適用します。練習回数は選択件数へ置き換えます。削除は各行から対象を明示して実行します。同一判定・選択・削除には、種類、比率、カメラ、回転、光源から作る見本キーを使用します。同じ種類でも向きや比率などが異なれば別のお気に入りとして保存できます。

### `Stroke`

描画した一本の線を、ツール、座標、色、太さ、透明度などの情報として保持します。座標はCanvasの実ピクセルではなく正規化した値で記録し、画面サイズ変更やSVG出力でも再利用します。

ストロークにはツールIDを保存し、評価時はツールレジストリに定義された役割へ変換します。

| 評価役割 | 対象ツール | 使用先 |
| --- | --- | --- |
| `draw` | ペン、点線 | 形状評価マスク |
| `shadow` | 影ペン | 投影影評価マスク |
| `erase` | 消しゴム | 形状・影の両マスクから削除 |
| `ignore` | 補助線 | 評価対象外 |
...
├─ size                     # 大きさ
├─ proportion               # 縦横比
├─ shadow                   # 投影影。対象外の場合はnull
├─ alignmentX/Y             # 中心合わせに使った平行移動量
└─ feedback                 # 優先度の高い改善案内
```

影を評価しない表示方式では`shadow`を`null`にし、影を描かなかった場合の0点と区別します。
...
練習結果と`PracticeSessionSnapshot`は現在のセッション中だけメモリに保持します。設定とお気に入りとは異なり、ページを閉じた後の練習履歴は保存していません。
...
sequenceDiagram
    ...
    S-->>P: 現在のShapePrompt
    P->>T: 表示用3D見本を描画
    P->>T: 影モードでは形状用・投影影用の評価見本も描画
    U->>D: ペン入力
    U->>P: 保存して次へ / 時間切れ
    P->>C: 表示用・形状用・影用Canvas、Stroke[]、練習モードを渡す
    C->>C: 評価とPNG・SVGを生成
    C-->>P: Attempt
    P->>S: Attemptを追加
    S-->>R: 全問完了時に結果画面へ
...
`features/results/attempt-capture.ts`は、見本画像の取得、自動評価、通常表示と中心合わせ表示のPNG・SVG生成をまとめて`Attempt`へ変換します。結果へ保存する表示用Canvasと、自動評価に使用するCanvasは分けて渡せます。「輪郭線と影」では、表示画像には完成見本を残し、形状評価には立体だけ、影評価には投影影だけを描いたCanvasを使用します。見本のみ表示モードでは描画画像を生成せず、未評価の結果を設定します。
...
- カメラ角度と立体回転の適用
- 表示方式と光源方向の適用
- 影モードで使用する、形状だけ・投影影だけの評価用シーンの生成
- Canvasサイズ変更時の再描画
- 不要になったThree.jsリソースの破棄

影モードでは`SampleRenderLayer`で同じ出題から描画対象を切り替えます。

| レイヤー | 内容 | 用途 |
| --- | --- | --- |
| `complete` | 立体、投影影、地面線 | 練習画面と結果保存 |
...
ペン、点線、補助線、影ペン、消しゴムは個別のツール定義として実装し、`tool-registry.ts`へ登録しています。
...
各ツールは、ストロークの生成、線幅、透明度、破線パターン、評価時の役割を定義します。通常線は`draw`、影ペンは`shadow`、消しゴムは`erase`、補助線は`ignore`として扱います。見た目の透明度から影を推測しないため、通常ペンの薄い線と投影影を混同しません。新しいツールは、ツール定義を追加してレジストリへ登録する形で拡張できます。
...
| モジュール | 責務 |
| --- | --- |
| `shape-evaluator.ts` | Canvasとストロークを形状・影の二値マスクへ変換し、各評価処理を接続する |
| `mask-evaluator.ts` | 形状を評価できるか判定し、中心合わせと4項目の採点順序を管理する |
| `mask-metrics.ts` | 中心合わせ、輪郭、傾き、大きさ、比率の計算を行う |
| `shadow-evaluator.ts` | 形状と同じ移動量を影へ適用し、投影影の一致率を計算する |
| `evaluation-result.ts` | 項目別得点から総合点とフィードバックを生成する |

1. 3D見本Canvasを解析用の小さな画像へ変換する。影モードでは形状だけと投影影だけのCanvasを使う
2. 輝度差から見本の輪郭マスクを生成する
3. 通常ペンから形状マスク、影ペンから影マスクを生成する
4. 見本と描画の中心を合わせる
5. 輪郭は4ピクセルの許容範囲で比較し、傾きは外接矩形内の3×3区画ごとに線方向を比較する。大きさと比率は中心合わせ前の外接矩形から計算する
6. 影モードでは同じ中心移動量を影へ適用し、影の方向・位置・広がりを計算する
7. 各評価値から総合得点とフィードバックを生成する

位置そのものは得点に含めず、中心を合わせた後も描画を拡大・縮小しません。そのため、描いた場所に左右されにくく、形と大きさを評価できます。影だけを個別に中央へ寄せることはせず、形状と同じ移動量を使うため、立体に対する影の相対位置は評価に残ります。

補助線は評価対象から除外し、影ペンは形状評価から、通常ペンは影評価から除外します。消しゴムは両方のマスクへ反映します。
...
- 練習設定
- お気に入りの`ShapePrompt`（種類、比率、カメラ角度、物体の回転、光源）

保存データを読み込むときは、型や値を確認します。不正な立体情報は除外してアプリを継続できるようにしています。

設定は`schemaVersion`と設定本体を持つラッパー形式で保存します。バージョン情報を持たない旧形式も読み込み、次回保存時に現行形式へ移行します。お気に入りは一覧全体に`schemaVersion`を持ち、各項目には`prompt`だけを保存します。お気に入りの旧形式は読み込みません。
...
- 横長画面ではCanvasへ残りの縦幅を配分し、練習画面下部の不要な余白を抑える
...
- 自動評価は2D画像上の形状と投影影を基準とした補助評価である
- ブラウザや入力機器による描画感覚の差がある
- Vinextはベータ版を利用している
- 現在は静的な単一画面アプリとして構成している

これらは、静的ホスティング、練習データを端末内で扱うこと、ブラウザだけで練習を完結させることを優先した結果でもあります。アクセス解析は練習機能から分離し、Google Analyticsだけを利用します。
