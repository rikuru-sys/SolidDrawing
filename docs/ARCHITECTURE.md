...
お気に入り自身の独立ID、見本の同一判定キー、`ShapePrompt`、練習時の`Settings`、追加日時を保持します。お気に入りの選択・削除には独立IDを使い、追加済みかどうかの判定には形状、比率、カメラ、回転、光源から作る見本キーを使います。再挑戦で出題IDが変わっても同じ見本と判定でき、同じシードと出題番号でも表示結果が違えば別の見本として扱えます。
