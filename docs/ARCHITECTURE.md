...
お気に入りは、再現対象の`sample`と登録時の練習条件を表す`savedPractice`を分けて保持します。`sample`には見本の同一判定キーと生成済みの`ShapePrompt`、`savedPractice`には正規化済みの`Settings`を保存します。さらに、お気に入り形式の`snapshotVersion`、登録時の`createdWithAppVersion`、追加日時を記録します。

現在は保存済みの見本と`savedPractice`の設定で1問を再練習します。見本と設定を独立させたことで、将来は現在の設定を適用する練習方法や、複数の見本をまとめて練習する方法を追加できます。お気に入りの選択・削除には独立IDを使い、追加済みかどうかの判定には形状、比率、カメラ、回転、光源から作る見本キーを使います。
