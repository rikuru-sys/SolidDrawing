...
GitHub Pages向けの静的ファイルをビルドし、`/SolidDrawing/`配下でテスト用サーバーから配信します。固定シード、1問、時間指定なしの設定をLocalStorageへ入れ、乱数や待ち時間に依存せず次を確認します。

- Canvasへ線を描き、保存して結果画面へ進める
- 見本のみモードで結果画面へ進める
- 結果をお気に入りへ追加して確認する
- `1280 × 551`で主要操作が表示領域に収まり、見本と描画Canvasの寸法が一致する
...
| E2E | `e2e` | 描画・見本のみ・結果・お気に入りの操作フロー、1280 × 551の配置 |
...
`1280 × 551`は自動確認していますが、縦長画面、スマートフォン、複数ブラウザの表示確認は未導入です。
