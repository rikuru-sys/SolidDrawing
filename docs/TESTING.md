...
- 結果をお気に入りへ追加して確認する
- お気に入りは同じ種類内の連番と光源方向で区別され、各行の削除前に対象名を含む確認が表示される
- 制限時間終了時に自動で結果へ進み、一時停止中は時間が進まない
- 時間指定なしで見本を途中で隠すと、15秒後に見本が非表示になる
- 複数の結果を前後に移動し、同じ立体・同じ設定で再練習できる
- お気に入りには種類・向き・比率・光源を含む立体情報だけが保存され、チェックした複数の立体を現在の練習設定で連続練習できる
...
- 固定した立方体の良い描画が90点以上、普通の描画が60～79点、悪い描画が49点以下になること
- 影の位置、方向、長さを変えた場合の評価順序
- 傾きが位置、大きさ、線幅だけでは下がらないこと
- 同じ方向の線でも、形の中の位置が異なる場合は傾きが下がること
...
実施時は[実機テスト仕様書](./MANUAL_TEST_SPEC.md)を使用し、端末・ブラウザ・入力機器ごとの操作手順、期待結果、合否、証拠を記録します。仕様書の未実施項目は、実機確認済みとして扱いません。
...
6. 最終問題の後に結果画面へ移動する
7. 横並びと重ね合わせを切り替えられる
8. 見本、描画、比較、平均評価付きの全結果を横並び・重ね合わせの両方で保存できる
9. お気に入りへ立体情報を追加し、チェックした複数の立体について向き・比率・光源を再現しながら現在の練習設定で連続練習できる
...
主要な練習フロー、時間切れ、一時停止、途中非表示、影ペン、比較モード、再練習、画像ダウンロードはChromiumで確認しています。PNGの余白調整では、5問の通常・重ね合わせ画像の寸法と、重ね合わせの先頭セルで立体が占める範囲・縁の余白を画素から検証します。対象は立方体の点線表示と影表示です。全セルの内容や全立体の画像差分までは自動確認していません。複数タッチはCDPで入力を再現していますが、実機のタッチ・液晶ペン入力、ブラウザを閉じた後の復元は未確認です。
