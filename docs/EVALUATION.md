...
評価マスクを作る前に、孤立した点や極端に短いストロークを除外します。固定の最小値に加え、同じ役割の最も大きなストロークに対する相対値を使い、線の長さと描画範囲の両方が小さい場合だけ描画ゴミと判定します。形状線と影線は別々に判定するため、形状線より短い影線も一律には除外しません。
...
傾きは、見本線と描画線から局所的な線方向を抽出し、0～180度の方向分布として比較します。位置、大きさ、描き漏らしによる距離は傾きの減点へ重複して含めず、それぞれ輪郭・大きさ・比率の項目で評価します。角や交差部のように方向を安定して求められない部分は傾き評価から除外します。
...
手描きの線をピクセル単位で完全一致させることは難しいため、輪郭の比較には`5`ピクセルの許容範囲を設けています。
...
傾き評価では、見本線と描画線の周辺ピクセルから局所的な線の方向を求め、それぞれの方向分布を比較します。線の位置、大きさ、描き漏らしによる距離は傾きの減点へ含めず、対応する別の評価項目で判定します。
...
局所方向を0～180度の18区間へ集計します。区間の境界付近にある線がわずかな差で別区間になる影響を抑えるため、隣接区間へ値を分配してから平滑化します。

正規化した見本と描画の方向分布を比較し、同じ方向の線が同程度含まれているほど高い一致率にします。線の方向は180度反転しても同じ傾きとして扱います。
