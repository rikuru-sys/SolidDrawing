...
type Props = { settings: Settings; onNext: () => void; disabled?: boolean };

export function PracticeFooter({ settings, onNext, disabled }: Props) {
  ...
  return <div className="practice-footer">
    ...
    {settings.time === null && <button className="button primary compact" type="button" disabled={disabled} onClick={onNext}>{hasDrawingCanvas ? '保存して次へ' : '次の見本へ'}</button>}
  </div>;
}
