...
type Props = {
  settings: Settings;
  tool: DrawingToolId;
  onChange: (patch: PracticePenStylePatch) => void;
};

export function DrawingStyleControls({ settings, tool, onChange }: Props) {
  const usesFixedShadowStyle = tool === 'shadow';
  return <div className="drawing-style-controls" role="group" aria-label="ペンの設定">
    ...
    {usesFixedShadowStyle
      ? <span className="shadow-pen-style-note">影色・濃さは固定</span>
      : <>
