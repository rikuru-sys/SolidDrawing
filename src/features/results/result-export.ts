...
type ExportKind = 'comparison' | 'drawing' | 'sample' | 'all';
...
export function allResultsCanvasSize(attemptCount: number) {
  ...
  return {
    width,
    height: headerHeight + rowCount * cardHeight + Math.max(0, rowCount - 1) * rowGap + 30,
    columnCount,
    rowCount,
  };
}
...
export function downloadAttemptDrawing(attempt: Attempt, index: number, date = new Date()) {
  downloadDataUrl(
    attempt.drawingImage,
    resultFileName('drawing', { index, shape: attempt.prompt.shape, date }),
  );
}
...
export function downloadAttemptSample(attempt: Attempt, index: number, date = new Date()) {
  downloadDataUrl(
    attempt.sampleImage,
    resultFileName('sample', { index, shape: attempt.prompt.shape, date }),
  );
}
...
export async function downloadAttemptComparison(options: {
  ...
  const { attempt, index, mode, overlayOpacity, date = new Date() } = options;
  ...
  const [sample, drawing] = await Promise.all([
    loadImage(attempt.sampleImage),
    loadImage(mode === 'overlay' ? attempt.alignedDrawingSvg : attempt.drawingSvg),
  ]);
  if (mode === 'overlay') {
  ...
  downloadDataUrl(
    ...
    resultFileName('comparison', { index, shape: attempt.prompt.shape, mode, date }),
  );
}
...
export async function downloadAllAttemptComparisons(options: {
  ...
  const { attempts, mode, overlayOpacity, date = new Date() } = options;
